...
export default function AssessmentDetail({ assessment: initialAssessment, unit: initialUnit, onBack }) {
  ...
  const markerPoints = buildMarkerPoints(assessment)
  const markerSource =
    markerPoints.length === 0
      ? null
      : Array.isArray(assessment.aiRubricSummary) && assessment.aiRubricSummary.length > 0
        ? 'AI decoded'
  ...
  return (
    <div className='screen'>
      ...
      <section>
        ...
        {markerPoints.length > 0 ? (
          <div>
            <span className='ai-badge'>{markerSource}</span>
...
function buildMarkerPoints(assessment) {
  const clean = (text = '') => text.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim()

  const aiPoints = Array.isArray(assessment.aiRubricSummary)
    ? assessment.aiRubricSummary.map((point) => clean(point)).filter(Boolean)
    : []
  if (aiPoints.length) return aiPoints.slice(0, 6)

  const rubricPoints = Array.isArray(assessment.rubric)
    ? assessment.rubric
        .map((criterion) => clean(criterion?.description || criterion?.longDescription || ''))
        .filter(Boolean)
    : []
  if (rubricPoints.length) return rubricPoints.slice(0, 6)

  const description = clean(assessment.description || '')
  ...
  const important = sentences.filter((sentence) =>
    /(must|required|should|need|criteria|assess|demonstrate|include|explain|analyse|develop|implement|evaluate|design)/i.test(
      sentence,
    ),
  )

  const chosen = important.length ? important : sentences
